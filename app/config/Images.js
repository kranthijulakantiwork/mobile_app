/* @flow */

export default {
  alerts_tab_active: require('app/assets/alerts_active.png'),
  alerts_tab_inactive: require('app/assets/alerts_inactive.png'),
  amount: require('app/assets/amount.png'),
  back: require('app/assets/back.png'),
  balance_negative: require('app/assets/balance_negative.png'),
  balance_positive: require('app/assets/balance_positive.png'),
  bill_name: require('app/assets/bill_name.png'),
  bills: require('app/assets/bills.png'),
  calendar: require('app/assets/calendar.png'),
  camera: require('app/assets/camera.png'),
  category: require('app/assets/category.png'),
  drinks: require('app/assets/drinks.png'),
  edit: require('app/assets/edit.png'),
  name: require('app/assets/name.png'),
  phone: require('app/assets/phone.png'),
  wallet_43: require('app/assets/wallet_43.png'),
  email: require('app/assets/email.png'),
  pencil: require('app/assets/pencil.png'),
  translate: require('app/assets/translate.png'),
  entertainment: require('app/assets/entertainment.png'),
  food: require('app/assets/food.png'),
  friends_tab_active: require('app/assets/friends_active.png'),
  friends_tab_inactive: require('app/assets/friends_inactive.png'),
  fuel: require('app/assets/fuel.png'),
  green_arrow: require('app/assets/green_arrow.png'),
  groceries: require('app/assets/groceries.png'),
  group: require('app/assets/group.png'),
  groups_tab_active: require('app/assets/groups_active.png'),
  groups_tab_inactive: require('app/assets/groups_inactive.png'),
  holiday: require('app/assets/holiday.png'),
  loan: require('app/assets/loan.png'),
  logo: require('app/assets/logo.png'),
  medicine: require('app/assets/medicine.png'),
  member: require('app/assets/member.png'),
  multiple_people: require('app/assets/multiple_people.png'),
  others: require('app/assets/category.png'),
  paid_by: require('app/assets/paid_by.png'),
  payee: require('app/assets/payee.png'),
  rent: require('app/assets/rent.png'),
  settings: require('app/assets/settings.png'),
  shopping: require('app/assets/shopping.png'),
  tick: require('app/assets/tick.png'),
  to_get: require('app/assets/to_get.png'),
  to_pay: require('app/assets/to_pay.png'),
  tracker_tab_active: require('app/assets/tracker_active.png'),
  tracker_tab_inactive: require('app/assets/tracker_inactive.png'),
  travel: require('app/assets/travel.png'),
  upi_link: require('app/assets/upi_link.png'),
  upi_logo: require('app/assets/upi_logo.png'),
  wallet: require('app/assets/wallet.png')
};
